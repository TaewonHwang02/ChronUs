...
const SchedulingMainPage = () => {
  ...
  const submitTimeSlots = async () => {
    ...
    if (minimumTimeSlots > 0 && totalSelectedMinutes < minimumTimeSlots) {
      alert(
        `You must select at least ${minimumTimeSlots} minutes. You selected ${totalSelectedMinutes} minutes.`
      );
      return;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const allParticipantsTimes = updatedMeeting.participants.flatMap(
          (p) => p.times
        );

        console.log(
          "Updated aggregated participant times:",
          allParticipantsTimes
        );
  ...
  console.log(
    "Aggregated Times before passing to GridOverlapDisplay:",
    aggregatedTimes
  );

  return (
    <div className="w-full h-full flex flex-col bg-primary py-10 relative overflow-y-auto">
      {/* Logo + Heading */}
      <div className="flex flex-col mt-[4%] tb:mt-[5px]">
        <div className="flex justify-center">
          <img src={registerLogo} alt="ChronUs Logo" className="w-auto h-10" />
        </div>
        <h1 className="font-poppins mt-10 text-[18px] tb:text-[2vw] leading-[3vw] text-white text-center">
